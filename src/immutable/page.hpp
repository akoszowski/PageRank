...
// Klasa reprezentująca pojedynczą stronę w naszym systemie zarządzania wiedzą.
class Page {
public:
    Page(std::string const& contentArg)
        : id("")
        , isIdComputed(false)
        , content(contentArg) // zawartość strony
        , links() // zbiór linków do treści powiązanych
    {
    }
...
private:
    mutable PageId id; // via mutable const pointer can change data member
